...
if __name__ == "__main__":
    ...
    new_thread4.start()
    new_thread5 = Thread(target=ServerService().recPool)
    new_thread5.start()
    new_thread6 = Thread(target=ServerService().recNewBlockVerify)
    new_thread6.start()
    new_thread7 = Thread(target=ServerService().recUpdatePool)
    new_thread7.start()
    new_thread8 = Thread(target=ServerService().recDeleteTrans)
    new_thread8.start()
    new_thread9 = Thread(target=ServerService().recFalseTrans)
    new_thread9.start()
    new_thread10 = Thread(target=ServerService().recRedirectCorrectTransactionsToPool)
    new_thread11 = Thread(target=ServerService().recSetFalseTransactionToZero)
    new_thread11.start()
    Pools().newUserPool()
    Signup().newFakeUser()
    new_thread10.start()
